...
```
    ...
    What does each row in the dataset represent?
    A specific La Quinta hotel in the US
    What are the variables?
    Address, city, state, zip, longitude and latitude
...
4.  Now take a look at the data.
    ...
    Creating an array with specific states of the US and analyze the codes which not belong there.
...
7.  Find the La Quinta locations that are outside the US, and figure out which country they are in.
    ...
    China, Canada, Colombia, New Zealand and Mexico.
...
```{r}
la_quinta_state_counts <- laquinta %>%
  group_by(state) %>%
  summarize(count = n())
la_quinta_state_counts <- la_quinta_state_counts %>%
  arrange(count)
print(la_quinta_state_counts)
```
```{r}
dennys_state_counts <- dennys %>%
  group_by(state) %>%
  summarize(count = n())
dennys_state_counts <- dennys_state_counts %>%
  arrange(count)
