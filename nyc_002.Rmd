...
```
```{r}
df7 |> ggplot(aes(x=time_hour,y=dep_delay)) + geom_point(alpha=.1)+stat_smooth()+ylim(-25,120)
```
```{r}
df7 |> ggplot(aes(x=hour+minute/60,y=dep_delay)) + geom_point(alpha=.1)+stat_smooth()+ylim(-25,120)
```
```{r}
df7 |> 
mutate(day_of_week=weekdays(time_hour)) |> ggplot(aes(x=hour+minute/60,y=dep_delay,color(day_of_week))) + geom_point(alpha=.1)+
stat_smooth()+
ylim(-25,120)
```
